...
public class _001_ZimunTorimVerifyRequestedServiceOptionsTest extends AbstractTest {
	...
	public Object[][] dataProviderPatients() {
		...
		try {
			...
			report.log("The as exception: " + ex);
			return null;
		}
	}
	
	
	// The next test - data driven verifies that the requested service select options are filled according to id number and year of birth
	...
	// According to the year of birth the patient is defined either as a baby or as a pregnant woman, and the optional services being filled accordingly
	...
	public void getServiceToSelectAccordingToPatientIdAndYearOfBirth (Patient patient) throws Exception {
		...
		try {
			// Step 1 - Fill services from file to expectedServices list
			...
			// Step 3 - Get the available values in the requested service select, and fill its values to actualServices list
			report.startLevel("Step 3 - Get the available values in the requested service select");
			...
			takeScreenshot("Screen shot of available services per patient type");
			report.endLevel();
			
			// Step 4 - Compare between 2 lists, and verify that the actual available values of the select, are identical to the expected values received from the csv file
			report.startLevel("Step 4 - Compare between 2 lists, and verify that the actual available values of the select, are identical to the expected values received from the csv file");
